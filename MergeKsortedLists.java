...
public class MergeKsortedLists {
   //make a new Pair class to be used otherwise it wont work
   public static class Pair implements Comparable<Pair> {
    ...
    // making the comparable function - now with this, PQ can decide which Pair is greater and which is not based on their values ofc
 ...
 public static ArrayList<Integer> mergeKSortedLists(ArrayList<ArrayList<Integer>> lists){
    ...
    for (int i = 0; i<lists.size(); i++) {
       Pair np = new Pair(i, 0, lists.get(i).get(0)); // get the first element value from every list in the arraylist- adds it to PQ
       pq.add(np); 
