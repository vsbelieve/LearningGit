...
public class IsBST {
  ...
  // Another Function which gives is tree is balanced or not

  static boolean isbal = true;
  public static int isBlncTree(Node node) { // returning height but changing isbal
    if(node == null){
      return 0;
    }

    int lh = isBlncTree(node.left);
    int rh = isBlncTree(node.right);
